...
void Forest::generateItems(int leafQuantity, int stickQuantity)
{
	for (int i = 0; i < leafQuantity; i++) 
	{
		...
		leaf->setPosition(vec3(rand() % landscapeWidth * 2 - landscapeWidth,
	...
	for (int i = 0; i < stickQuantity; i++)
	{
		...
		stick->setPosition(vec3(rand() % landscapeWidth * 2 - landscapeWidth,
			0, rand() % landscapeHeight * 2 - landscapeHeight));
...
int Forest::StartSimulation()
{
	...
	//добавим поверхность
	Landscape* landscape = new Landscape(window, camera, res, par, &objects, landscapeWidth, landscapeHeight);
	...
	Queen* queen = new Queen(camera, res, par, &objects, window, vec3(-4, 0, -3));
